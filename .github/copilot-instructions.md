...
```javascript
{
    ...
    height: number,
    color: string,     // background color (#ffffff) - per-node customization
    textColor: string, // text color (#333333 or auto-computed) - per-node customization
    ...
    searchDomains: string[], // optional AI search domain filter (e.g., ['github.com'])
    aiRecommendations: object[] // optional AI recommendations array (if AI feature enabled)
}
```

Rendering pipeline in `canvas.js` → `nodes.js`:
1. `drawCanvas()` orchestrates: clear → grid → connections → nodes → selection box → notification badges (if AI enabled)
...
6. **AI notification badges** (🔔): Rendered in top-right corner if node has unread recommendations
...
- **Save pattern**: `saveMindmap()` prompts for name, stores as `mindmap_file_${id}` with JSON.stringify (note: `mindmap_file_` prefix, not `mindmap_`)
...
- **Storage clear**: Use `CLEAR_STORAGE.md` instructions if migration needed (old bug caused array reference sharing)

**Cloud sync available but NOT integrated**: Files `supabase-config.js`, `auth.js`, `cloud-storage.js` exist but are NOT loaded in `index.html`. See SUPABASE_SETUP.md for future integration.
...
- **`ai-recommendations.js`** (393 lines, NOT loaded): AI-powered recommendations via Tavily API - searches web for node-related content, domain filtering, notification badges. Ready for integration.
- **`supabase-config.js`, `auth.js`, `cloud-storage.js`** (NOT loaded): Supabase cloud sync infrastructure - email/OAuth authentication, cloud save/load. Complete but unintegrated. See SUPABASE_SETUP.md.
...
### Optional Features (Ready but Not Integrated)

**AI Recommendations System** (`ai-recommendations.js`):
- **Status**: Complete implementation but NOT loaded in `index.html`
- **Functionality**: Tavily API integration for web search based on node title/content
- **Features**: Domain filtering (`searchDomains` array), notification badges (🔔), recommendation management
- **Data structure**: Node property `aiRecommendations: [{title, url, description, timestamp, read}]`
- **Storage**: API key in `localStorage.getItem('ai_api_key')`, enabled flag in `ai_recommendations_enabled`
- **Integration steps**: Add `<script src="js/ai-recommendations.js">` before `main.js`, wire up UI buttons/modals
- **Dependencies**: None (uses Fetch API), requires user's Tavily API key

**Supabase Cloud Sync** (`supabase-config.js`, `auth.js`, `cloud-storage.js`):
- **Status**: Complete infrastructure but NOT loaded in `index.html`
- **Functionality**: Multi-device sync, user authentication (email/Google/GitHub OAuth)
- **Database schema**: `mindmaps` table with RLS (Row Level Security) policies
- **Current keys**: In `supabase-config.js` - `SUPABASE_URL` and `SUPABASE_ANON_KEY` (already configured)
- **Integration steps**: 
  1. Add Supabase CDN: `<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>`
  2. Load scripts: `supabase-config.js` → `auth.js` → `cloud-storage.js` before `main.js`
  3. Wire up auth UI (login/signup modals), cloud save/load buttons
- **See**: SUPABASE_SETUP.md for complete setup guide

### localStorage Keys
- `mindmap_recent_files`: Array of `{id, name, timestamp, favorite}` objects
- `mindmap_file_${uuid}`: Each saved mindmap as JSON string `{nodes[], connections[]}` (note: `mindmap_file_` prefix)
- `language`: Current UI language ('ko' or 'en')
- `theme`: Current theme ('dark' or 'light')
- `snapToGrid`: Boolean for grid snapping preference
- `ai_api_key`: Tavily API key (if AI recommendations enabled)
- `ai_recommendations_enabled`: Boolean flag for AI feature toggle
- **Legacy**: Old files used `mindmap_${uuid}` without `_file_` - see CLEAR_STORAGE.md for migration
